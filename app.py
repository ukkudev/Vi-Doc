...
diseases_doctor = pd.read_csv("data/symptom_doctors.csv")
diseases_doctor['Disease'] = diseases_doctor['Disease'].apply(lambda x: x.lower().strip(" "))
    ...
    if sentence.replace(".", "").replace("!","").lower().strip() == "done":
        ...
        if not user_symptoms:
            response_sentence = random.choice(
                ...
                "Vi-Doc can't know the disease if there are no symptoms...",
            ...
            doctor = diseases_doctor.loc[diseases_doctor['Disease'] == disease.strip(" ").lower(), 'doctors'].iloc[0]
            ...
            response_sentence = "You may consult " + "<b>" + doctor + "</b>" + ". <br><br> <i>Description: " + description + "</i>" + "<br><br><b>"+ precautions + "</b>"
            ...
            if np.mean(severity) > 4 or np.max(severity) > 5:
                response_sentence = response_sentence + "<br><br>Since Vi-Doc isn't a real doctor, we recomed you to consult a DOCTOR."
        ...
        if prob > .5:
            response_sentence = f"Hmm, I'm pretty sure this is " + "<b>" + symptom + "</b>" + " please enter more symptoms or type" + "<b>" + " done" + "</b>" + " to see the results"
            user_symptoms.add(symptom)
        else:
            response_sentence = "I'm sorry, but I couldn't understand you."
